...
void MyTcpServer::slotServerRead()
{
    while (mTcpSocket->bytesAvailable() > 0)
    {
        ...
        if (message.startsWith("auth&")) {
            ...
            ServerFunction::handleProfile(mTcpSocket, message);
        } else if (message.startsWith("blacklist&")) {
            ServerFunction::handleBlacklist(mTcpSocket, message);
        } else if (message.startsWith("addblacklist&")) {
            ServerFunction::handleBlacklistAdd(mTcpSocket, message);
        }
        else {
